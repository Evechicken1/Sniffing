...
savefolder = r'C:\Users\xaand\Documents\PhD\Experiments\Ephys OFC\Analysis\Sniffing'
...
if savefig:
    ...
    filename = f"{savefolder}/mean sniffing to first 3 presentations_sniffs_{sess_ids}_{now}.png"
...
for m in range(nmice):
    ...
    opto_odors = (sniffs[m]['trial_opto'] == 1) & (sniffs[m]['trial_occur'] <= 3)& (sniffs[m]['trial_blank'] == 0)
...
if savefig:
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")  # e.g. 20250819_184512
    filename = f"{savefolder}\Sniffing to first 3_8_8 presentations_sniffs_{sess_ids}.png"
    plt.savefig(filename, dpi=400, bbox_inches="tight")
