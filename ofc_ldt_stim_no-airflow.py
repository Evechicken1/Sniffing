...
#paths_list = [f"X:\\haeslerlabwip2024\\postprocessed_data\\{sess_id}" for sess_id in sess_ids]
...
ecolor = ['purple','violet'] 
graph = ["no opto vs opto"]
markercolors = ['purple','violet'] 
BoI = range(5, 8)
comp_grps= ["no opto vs opto"]
...
for m in range(nmice):
    ...
    no_opto_trials = [(sniffs[m]['trial_opto'] == 0) for o in range(len(odors))]
    no_opto_trials = [i for i in no_opto_trials if sum(i)>0]

    opto_trials = [(sniffs[m]['trial_opto'] == 1) for o in range(len(odors))]
    opto_trials = [i for i in opto_trials if sum(i)>0]
    
    notopto = len(no_opto_trials) >0
    ...
    conditions = [no_opto_data, opto_data]
...
for m in range(nmice):
    ...
    trial_idx = sniffs[m]["trial_idx"] - 1 
    no_opto_odors = sniffs[m]['trial_opto'] == 0
    opto_odors = sniffs[m]['trial_opto'] == 1

    no_opto_idxs = trial_idx[no_opto_odors]
    opto_idxs = trial_idx[opto_odors]
    
    no_opto_data_tmp = np.array([len(np.where((i >t_range[0]) & (i < t_range[1]))[0]) for i in sniffs[m]['ml_inh_onsets'][no_opto_idxs]])/t_time
    opto_data_tmp = np.array([len(np.where((i >t_range[0]) & (i < t_range[1]))[0]) for i in sniffs[m]['ml_inh_onsets'][opto_idxs]])/t_time

    no_opto_bl = np.array([len(np.where((i > bl_range[0]) & (i < bl_range[1]))[0]) for i in sniffs[m]['ml_inh_onsets'][no_opto_idxs]])/bl_time
    opto_bl = np.array([len(np.where((i > bl_range[0]) & (i < bl_range[1]))[0]) for i in sniffs[m]['ml_inh_onsets'][opto_idxs]])/bl_time

    no_opto_data = (no_opto_data_tmp - no_opto_bl).mean()
    opto_data = (opto_data_tmp - opto_bl).mean()

    no_opto.append(no_opto_data)
    opto.append(opto_data)

data = np.array([no_opto, opto])
...
data = np.array([no_opto, opto])   # now matches graph order
