...
#!!! Session ids should be of one type (here: non-opto, novel/familiar only)

# XT026, XT027, XT029 - novel & familiar odors only (no opto)
animals = ['XT026', 'XT027', 'XT029']
dates = ['260512', '260513', '260514', '260515']
...
savefolder = r"C:\Users\xaand\Documents\PhD\Experiments\Ephys LDTg\Analysis\Sniffing"
...
if savefig:
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{savefolder}\Sniffing to first 3 presentations_sniffs_{sess_label}.png"
    plt.savefig(filename, dpi=400, bbox_inches="tight")
...
if savefig:
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{savefolder}\Sniffing to first 3_all presentations_sniffs_{sess_label}.png"
    plt.savefig(filename, dpi=400, bbox_inches="tight")
