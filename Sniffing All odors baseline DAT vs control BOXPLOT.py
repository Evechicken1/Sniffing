...
#    print("graph", g)
#    baseline[g] = np.mean(mean_arr[g][:,0:3])
    mean[g] = np.mean(np.mean(sniff_lst[g,:], axis = 1)[:, baseline_bins], axis = 1) # - baseline
    ...
    plt.title("Baseline sniffing [all trials]")
...
print(stats.ttest_rel(mean[0],mean[1]),"baseline sniffing all trials")

print(mean)
#%% Boxplot of baseline of all trials
